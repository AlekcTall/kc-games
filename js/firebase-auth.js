...
async function firebaseRegister(email, password, username, department) {
  try {
    ...
    updateAuthUI(user);
    return userData;
  } catch (error) { console.error('Ошибка регистрации:', error); throw error; }
}
...
async function firebaseLogin(email, password) {
  try {
    ...
    const user = userCredential.user;
    if (!user.emailVerified) { await user.sendEmailVerification(); await auth.signOut(); throw new Error('Email не подтверждён. Новое письмо отправлено.'); }
    const doc = await db.collection('users').doc(user.uid).get();
    if (doc.exists) {
      const data = doc.data();
      if (data.disabled) { await auth.signOut(); throw new Error('Учётная запись заблокирована.'); }
      const userData = {
        uid: user.uid, email: user.email, username: data.username, department: data.department,
        points: data.points || 0, lokoin_balance: data.lokoin_balance || 0, purchasedItems: data.purchasedItems || [],
        role: data.role || 'user', description: data.description || '', achievements: data.achievements || [],
      ...
      setCurrentUser(userData); updateAuthUI(user); await updateLastActive(user.uid);
      const loginResult = await processDailyLogin(user.uid);
      if (loginResult) { userData._dailyReward = loginResult; setCurrentUser(userData); }
      return userData;
    } else throw new Error('Документ пользователя не найден');
  } catch (error) { console.error('Ошибка входа:', error); throw error; }
}

function firebaseLogout() { auth.signOut().then(() => { localStorage.removeItem('krugames_currentUser'); updateAuthUI(null); window.location.href = 'index.html'; }).catch(e => console.error(e)); }
...
function getDailyReward(day) {
  const rewards = {
    1: { points: 2, lokoin: 1, label: 'День 1' }, 2: { points: 3, lokoin: 1, label: 'День 2' },
    3: { points: 5, lokoin: 2, label: 'День 3' }, 4: { points: 5, lokoin: 2, label: 'День 4' },
    5: { points: 8, lokoin: 3, label: 'День 5' }, 6: { points: 8, lokoin: 3, label: 'День 6' },
...
function getMoscowDate() { const now = new Date(); return new Date(now.getTime() + 3 * 3600000).toISOString().slice(0, 10); }
function getYesterdayMoscow() { const d = new Date(Date.now() + 3 * 3600000); d.setDate(d.getDate() - 1); return d.toISOString().slice(0, 10); }
...
async function processDailyLogin(uid) {
  ...
  try {
    ...
    const today = getMoscowDate(); const yesterday = getYesterdayMoscow();
    if (dailyLogin.lastLoginDate === today) return null;
    let newStreak = dailyLogin.streak || 0;
    if (dailyLogin.lastLoginDate === yesterday) newStreak += 1; else newStreak = 1;
    const reward = getDailyReward(newStreak);
    const loginHistory = dailyLogin.loginHistory || []; loginHistory.push(today);
    const newDailyLogin = { lastLoginDate: today, streak: newStreak, longestStreak: Math.max(dailyLogin.longestStreak || 0, newStreak), totalLogins: (dailyLogin.totalLogins || 0) + 1, loginHistory: loginHistory.slice(-60) };
    const oldPoints = data.points || 0; const newPoints = oldPoints + reward.points;
    const oldLokoin = data.lokoin_balance || 0; const newLokoin = oldLokoin + reward.lokoin;
    await userRef.update({ dailyLogin: newDailyLogin, points: newPoints, lokoin_balance: newLokoin });
    if (typeof addNotification === 'function') await addNotification(uid, `Ежедневный вход (${reward.label}): +${reward.points} баллов, +${reward.lokoin} локоинов`, 'game', 'profile.html');
    if (typeof checkAndAwardAchievements === 'function') await checkAndAwardAchievements();
    const current = getCurrentUser(); if (current) { current.points = newPoints; current.lokoin_balance = newLokoin; current.dailyLogin = newDailyLogin; setCurrentUser(current); }
...
async function firebaseUpdateProfile(uid, data) {
  try {
    ...
    const current = getCurrentUser();
    if (current && (current.uid === uid || current.id === uid)) { Object.assign(current, data); setCurrentUser(current); }
    return true;
...
async function addPointsToCurrentUser(points, gameId = null) {
  ...
  try {
    ...
    const oldPoints = data.points || 0; const newPoints = oldPoints + actualPoints;
    const oldLokoin = Math.floor(oldPoints / 10); const newLokoin = Math.floor(newPoints / 10);
    const delta = newLokoin - oldLokoin;
    const currentLokoinBalance = data.lokoin_balance || 0; const newLokoinBalance = currentLokoinBalance + delta;
    const updateData = { points: newPoints };
    if (gameId) {
      const completedGames = data.completedGames || [];
      if (!completedGames.includes(gameId)) { completedGames.push(gameId); updateData.completedGames = completedGames; }
    ...
    if (delta > 0 && typeof addLokoinNotification === 'function') addLokoinNotification(user.uid, delta).catch(e => console.error(e));
    const current = getCurrentUser();
    if (current) { current.points = newPoints; if (delta > 0) current.lokoin_balance = newLokoinBalance; if (updateData.completedGames) current.completedGames = updateData.completedGames; if (updateData.gameHistory) current.gameHistory = updateData.gameHistory; setCurrentUser(current); }
    return true;
...
async function syncEasterEggsToFirestore(easterEggs) {
  ...
  try { await db.collection('users').doc(user.uid).update({ easterEggsFound: easterEggs }); const c = getCurrentUser(); if (c) { c.easterEggsFound = easterEggs; setCurrentUser(c); } } catch (e) { console.error(e); }
}

async function syncAchievementsToFirestore(achievements) {
  const user = auth.currentUser; if (!user) return;
  try { await db.collection('users').doc(user.uid).update({ achievements: achievements }); const c = getCurrentUser(); if (c) { c.achievements = achievements; setCurrentUser(c); } } catch (e) { console.error(e); }
}
